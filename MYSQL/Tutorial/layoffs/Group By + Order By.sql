...
SELECT 
    gender, MIN(number), MAX(number), COUNT(number), AVG(number)
FROM
    departments_data
GROUP BY gender;
...
#So let's try it out with our employee table
...
FROM
    departments_data
ORDER BY gender , employment_status;

SELECT 
    *
FROM
    departments_data
ORDER BY gender DESC , employment_status DESC;
