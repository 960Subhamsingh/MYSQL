...
FROM
    employee_salary sal
        RIGHT JOIN
    employes dem ON dem.employee_id = sal.employee_id;
...
FROM
    employee_salary emp1
        JOIN
    employee_salary emp2 ON emp1.employee_id = emp2.employee_id;
...
alter table departments_data drop column  Dep_id;
alter table departments_data add dept_id int;
INSERT INTO departments_data (dept_id)
SELECT eeid
FROM employees;
...
INNER JOIN employee_salary sal
	ON dem.`Annual Salary`= sal.salary
LEFT JOIN department_data dept
